...
import { withSongCredits, withSongCreditsList } from './song-credits.js';
...
export async function handlePublicApi(request, env, url) {
  ...
  if (url.pathname === '/api/songs') return json({ songs: withSongCreditsList(await listPublishedSongs(env.DB)) });
  ...
  if (songMatch) {
    ...
    return json({ song: withSongCredits(song) });
  }
  ...
  if (albumMatch) {
    ...
    return json({ album, songs: withSongCreditsList(songs) });
  }
  return null;
}
