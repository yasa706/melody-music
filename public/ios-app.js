...
export function createMelodyIOSAudio({
  ...
  return Object.freeze({
    ...
    async setQueue(queue = [], index = 0) { return call('setQueue', { queue: queue.map((song) => normalizeSong(song, origin)), index: Math.max(0, Number(index) || 0) }); },
    async play(song) { return call('play', { song: normalizeSong(song, origin) }); },
    async pause() { return call('pause'); },
    async resume() { return call('resume'); },
    async seek(seconds) { return call('seek', { seconds: Math.max(0, Number(seconds) || 0) }); },
    async next() { return call('next'); },
    async previous() { return call('previous'); },
    async getState() { return call('getState'); },
  });
}
...
async function deleteAccount(button) {
  const confirmed = globalThis.window?.confirm?.('确定要永久删除账号吗？收藏、歌单和播放记录也会一起删除。此操作无法撤销。');
  if (!confirmed) return;
  const original = button.textContent;
  button.disabled = true;
  button.textContent = '正在删除…';
  try {
    const response = await fetch('/api/auth/account', { method: 'DELETE', credentials: 'same-origin', headers: { Accept: 'application/json' } });
    if (!response.ok) throw new Error('删除账号失败，请重新登录后再试');
...
export function installIOSChrome(doc = globalThis.document) {
  ...
  if (!doc.querySelector?.('link[data-ios-mobile-style]')) {
    ...
    link.rel = 'stylesheet'; link.href = '/ios-mobile.css'; link.dataset.iosMobileStyle = 'true'; doc.head?.appendChild(link);
  }
  installIOSLaunchSplash(doc);
  installLegalLinks(doc);
  const nav = doc.createElement('nav');
  nav.id = 'iosBottomNav'; nav.className = 'ios-bottom-nav'; nav.setAttribute('aria-label', 'iPhone 主导航');
  nav.innerHTML = `<button class="ios-bottom-nav-button" type="button" data-ios-tab="home" aria-current="page"><span>⌂</span><span>首页</span></button><button class="ios-bottom-nav-button" type="button" data-ios-tab="albums"><span>▦</span><span>专辑</span></button><button class="ios-bottom-nav-button" type="button" data-ios-tab="search"><span>⌕</span><span>搜索</span></button><button class="ios-bottom-nav-button" type="button" data-ios-tab="my"><span>♪</span><span>我的</span></button>`;
  doc.body.appendChild(nav);
  const setCurrent = (button) => { nav.querySelectorAll('[data-ios-tab]').forEach((item) => item.removeAttribute('aria-current')); button.setAttribute('aria-current', 'page'); };
  nav.addEventListener('click', (event) => {
    const button = event.target.closest?.('[data-ios-tab]'); if (!button) return; setCurrent(button);
    const tab = button.dataset.iosTab;
    if (tab === 'home') globalThis.window?.scrollTo?.({ top: 0, behavior: 'smooth' });
    else if (tab === 'albums') doc.querySelector?.('.albums-section')?.scrollIntoView?.({ behavior: 'smooth', block: 'start' });
    else if (tab === 'search') { const input = doc.getElementById?.('searchInput'); input?.scrollIntoView?.({ behavior: 'smooth', block: 'center' }); setTimeout(() => input?.focus?.(), 250); }
    else if (tab === 'my') { const userButton = doc.getElementById?.('userMenuButton'); const loginButton = doc.getElementById?.('loginButton'); if (userButton && !userButton.closest?.('[hidden]')) userButton.click?.(); else loginButton?.click?.(); }
  });
  const miniPlayer = doc.querySelector?.('.player-song');
  miniPlayer?.addEventListener?.('click', () => { doc.querySelector?.('.now-panel')?.scrollIntoView?.({ behavior: 'smooth', block: 'start' }); });
  return true;
}

function resolveNativePlugin() { return globalThis.window?.Capacitor?.Plugins?.NativeAudio || null; }

if (typeof window !== 'undefined') {
  const plugin = resolveNativePlugin();
  window.MelodyIOSAudio = createMelodyIOSAudio({ plugin, target: window, origin: window.location.origin });
  if (plugin) {
