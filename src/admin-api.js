...
export function normalizeSong(input = {}) {
  const out = {
    ...
    lyricist: String(input.lyricist || '').trim(), composer: String(input.composer || '').trim(),
...
async function handleSongs(request, env, url) {
  ...
  if (request.method === 'POST' && !id) {
    ...
    const result = await env.DB.prepare(`INSERT INTO songs (title,artist,album,lyricist,composer,album_id,category_id,audio_type,audio_url,cover_type,cover_url,lyrics_lrc,duration_seconds,is_published,sort_order) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`)
      .bind(s.title,s.artist,s.album,s.lyricist,s.composer,s.album_id,s.category_id,s.audio_type,s.audio_url,s.cover_type,s.cover_url,s.lyrics_lrc,s.duration_seconds,s.is_published,s.sort_order).run();
  ...
  if (request.method === 'PUT' && id) {
    ...
    const normalized = normalizeSong(body); if (normalized.error) return json({ error: { code: 'VALIDATION_ERROR', message: normalized.error } }, 400);
    const s = normalized.value;
    await env.DB.prepare(`UPDATE songs SET title=?,artist=?,album=?,lyricist=?,composer=?,album_id=?,category_id=?,audio_type=?,audio_url=?,cover_type=?,cover_url=?,lyrics_lrc=?,duration_seconds=?,is_published=?,sort_order=?,updated_at=CURRENT_TIMESTAMP WHERE id=?`)
      .bind(s.title,s.artist,s.album,s.lyricist,s.composer,s.album_id,s.category_id,s.audio_type,s.audio_url,s.cover_type,s.cover_url,s.lyrics_lrc,s.duration_seconds,s.is_published,s.sort_order,id).run();
...
async function handleAlbums(request, env, url) {
  ...
  if (request.method === 'PUT' && id) {
    ...
    const album=await env.DB.prepare('SELECT * FROM albums WHERE id=?').bind(id).first(); album.song_ids=a.song_ids; return json({album});
  }
  if (request.method === 'DELETE' && id) {
...
async function handleDashboard(request, env, url) {
  if (url.pathname !== '/api/admin/dashboard') return null;
  const auth = await authorize(request, env); if (auth.response) return auth.response;
  const [songs,published,playlists,categories,albums] = await Promise.all([
    env.DB.prepare('SELECT COUNT(*) AS n FROM songs').first(), env.DB.prepare('SELECT COUNT(*) AS n FROM songs WHERE is_published=1').first(), env.DB.prepare('SELECT COUNT(*) AS n FROM playlists').first(), env.DB.prepare('SELECT COUNT(*) AS n FROM categories').first(), env.DB.prepare('SELECT COUNT(*) AS n FROM albums').first()
  ]);
  return json({dashboard:{songs:songs?.n||0,published:published?.n||0,drafts:(songs?.n||0)-(published?.n||0),playlists:playlists?.n||0,categories:categories?.n||0,albums:albums?.n||0}});
}

async function handleUpload(request, env, url) {
  if (url.pathname !== '/api/admin/upload') return null;
  ...
  try { const form=await request.formData(); const file=form.get('file'); const kind=String(form.get('kind')||''); const stored=await storeUpload(env,file,kind); return json(stored,201); }
  catch(e){ if(e instanceof UploadError)return json({error:{code:e.code,message:e.message}},e.status); throw e; }
}

async function handleMediaDelete(request, env, url) {
  const match=url.pathname.match(/^\/api\/admin\/media\/(.+)$/); if(!match)return null; if(request.method!=='DELETE')return json({error:{code:'METHOD_NOT_ALLOWED',message:'Method not allowed'}},405); const auth=await authorize(request,env,true);if(auth.response)return auth.response; const key=decodeURIComponent(match[1]); await deleteUpload(env,key); return new Response(null,{status:204});
}

export async function handleAdminApi(request, env, url = new URL(request.url)) {
  const handlers=[handleSongs,handleAlbums,handleCategories,handlePlaylists,handleDashboard,handleUpload,handleMediaDelete];
  for(const handler of handlers){const res=await handler(request,env,url);if(res)return res;}
  return null;
}
